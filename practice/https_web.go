...
func YourListenAndServeTLS(addr, certFile, keyFile string, handler http.Handler) error {
	config := &tls.Config{
		Rand: rand.Reader,
		Time: time.Now,
...
func YourLoadX509KeyPair(cerFile, keyFile string) (cert tls.Certificate, err error) {
	certPEMBlock, err := ioutil.ReadFile(cerFile)
	if err != nil {
		return
	}
	certDERBlock, restPEBlock := pem.Decode(certPEMBlock)
	if certDERBlock == nil {
		err = errors.New("failed to parse certificate PEM data")
		return
	}

	certDERBlockChain, _ := pem.Decode(restPEBlock)
	...
	if x509Cert.PublicKeyAlgorithm != x509.RSA || x509Cert.PublicKey.(*rsa.PublicKey).N.Cmp(key.PublicKey.N) != 0 {
		err = errors.New("private key does not match public key")
		return
	}
	return
}
