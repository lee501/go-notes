...
func main() {
	//初始化
	s1 := Student{Person{"mike", 'm', 18}, 1, "sz", "ss"} //顺序初始化，注意Person的初始化方法

	s2 := Student{Person: Person{name: "lee"}, id: 1}  //部分成员初识化

	//成员操作
	var s3 Student
	s3.name = "lee" //同名字段默认给外层成员赋值
	s3.Person.name = "anne" //同名字段给内层成员赋值
	s3.sex = 0 //等同于s3.Person.sex
	//Person为结构体，是一个匿名字段
	s3.Person = Person{"lee", 1, 22}

	//非结构体匿名字段的赋值
	t := Teacher{int: 1, mystr: "hello"}

	//	结构体指针的初始化
	s4 := Student1{&Person{"mike", 1, 18}, 1}
	// 声明变量方式
	var s5 Student1
	s5.Person = new(Person) //new 一个空间
	s5.Person.name = "lee" //等同于s5.name
}
